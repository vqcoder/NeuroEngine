"""NeuroScore taxonomy, diagnostics pathways, product rollups, and calibration schemas."""
    ...
    def validate_payload(self):
        if self.pathway != AttentionalSynchronyPathway.insufficient_data:
            if self.global_score is None:
                raise ValueError("global_score must be provided when synchrony pathway has data")
            if self.confidence is None:
                raise ValueError("confidence must be provided when synchrony pathway has data")
        return self
...
class NarrativeControlDiagnostics(BaseModel):
    ...
    def validate_payload(self):
        if self.pathway != NarrativeControlPathway.insufficient_data:
            if self.global_score is None:
                raise ValueError("global_score must be provided when narrative pathway has data")
            if self.confidence is None:
                raise ValueError("confidence must be provided when narrative pathway has data")
        return self
...
class BlinkTransportDiagnostics(BaseModel):
    ...
    def validate_payload(self):
        if self.pathway not in {
            ...
            if self.global_score is None:
                raise ValueError("global_score must be provided when blink transport pathway has data")
            if self.confidence is None:
                raise ValueError("confidence must be provided when blink transport pathway has data")
        return self
...
class RewardAnticipationDiagnostics(BaseModel):
    ...
    def validate_payload(self):
        if self.pathway != RewardAnticipationPathway.insufficient_data:
            if self.global_score is None:
                raise ValueError("global_score must be provided when reward anticipation pathway has data")
            if self.confidence is None:
                raise ValueError("confidence must be provided when reward anticipation pathway has data")
        return self
...
class BoundaryEncodingDiagnostics(BaseModel):
    ...
    def validate_payload(self):
        if self.pathway != BoundaryEncodingPathway.insufficient_data:
            if self.global_score is None:
                raise ValueError("global_score must be provided when boundary encoding pathway has data")
            if self.confidence is None:
                raise ValueError("confidence must be provided when boundary encoding pathway has data")
        return self
...
class SocialTransmissionDiagnostics(BaseModel):
    ...
    def validate_payload(self):
        if self.pathway != SocialTransmissionPathway.insufficient_data:
            if self.global_score is None:
                raise ValueError("global_score must be provided when social transmission pathway has data")
            if self.confidence is None:
                raise ValueError("confidence must be provided when social transmission pathway has data")
        return self
...
class SelfRelevanceDiagnostics(BaseModel):
    ...
    def validate_payload(self):
        if self.pathway != SelfRelevancePathway.insufficient_data:
            if self.global_score is None:
                raise ValueError("global_score must be provided when self relevance pathway has data")
            if self.confidence is None:
                raise ValueError("confidence must be provided when self relevance pathway has data")
        return self
...
class AuFrictionDiagnostics(BaseModel):
    ...
    def validate_payload(self):
        if self.pathway != AuFrictionPathway.insufficient_data:
            if self.global_score is None:
                raise ValueError("global_score must be provided when AU friction pathway has data")
            if self.confidence is None:
                raise ValueError("confidence must be provided when AU friction pathway has data")
        return self
...
class CtaReceptionDiagnostics(BaseModel):
    ...
    def validate_payload(self):
        if self.pathway != CtaReceptionPathway.insufficient_data:
            if self.global_score is None:
                raise ValueError("global_score must be provided when CTA reception pathway has data")
            if self.confidence is None:
                raise ValueError("confidence must be provided when CTA reception pathway has data")
        return self


# ---------------------------------------------------------------------------
# Aggregate Metrics (references all diagnostics above)
# ---------------------------------------------------------------------------
...
class NeuroScoreFamilies(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_family_machine_names(self):
        expected = {
...
class NeuroRollupFamilies(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_rollup_machine_names(self):
        expected = {
...
class IncrementalityExperimentResultIn(BaseModel):
...
class SyntheticLiftCalibrationStateRead(BaseModel):
    model_version: str = Field(min_length=1)
    truth_layer: str = Field(min_length=1, max_length=64)
    observation_count: int = Field(ge=0)
    lift_bias_pct: float
    iroas_bias: float
    uncertainty_scale: float = Field(ge=0)
    updated_at: Optional[datetime] = None


class SyntheticLiftCalibrationSyncRequest(BaseModel):
    experiments: List[IncrementalityExperimentResultIn] = Field(default_factory=list)
    apply_calibration_updates: bool = True


class SyntheticLiftCalibrationSyncResponse(BaseModel):
    ...
    calibration_state: SyntheticLiftCalibrationStateRead


class SyntheticLiftCalibrationStatusResponse(BaseModel):
    total_experiments: int = Field(ge=0)
    pending_experiments: int = Field(ge=0)
    last_calibration_applied_at: Optional[datetime] = None
    calibration_state: SyntheticLiftCalibrationStateRead
